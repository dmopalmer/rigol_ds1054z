...
class rigol_ds1054z:
    ...
    def setup_channel(
        self, channel=1, on=True, offset_divs=0.0, volts_per_div=1.0, probe=10.0
    ):
        if on:
            # fmt: off
    ...
    def setup_i2c_decode(
        ...
        if on == 0:
            ...
            self.inst.write(':DEC' + str(decode_channel) + ':FORM ' + encoding.upper())
    ...
    def run_trigger(self, mode:Optional[Literal['AUTO', 'NORMAL', 'SINGLE']]):
        if mode is not None:
            self.inst.write(f":TRIGGER:SWEEP {mode.upper()}")
        self.inst.write(":RUN")
        time.sleep(3)
    ...
    def setup_mem_depth(self, memory_depth=12e6):
        ...
        Args:
            memory_depth (_type_, optional): Requested depth. Defaults to 12e6.

        Returns:
            float: Depth as written
        """
        if self.status() == "STOP":
        ...
        if str(memory_depth).upper() == "AUTO":
            ...
            rigol_depths = sorted(
                np.concatenate([digit * 10 ** np.arange(3, 7) for digit in (3, 6, 12)])
            ) + [24 * 10**6]
        ...
        if self.verbose:
            print(
                f"Acquire memory depth set to {newdepth} samples ({memory_depth} requested)"
            )
        return newdepth

    def set(self, datum:str, value:Optional[str] = None, readback=True):
        writestring = datum
    ...
    def get_scales(
        self, istart=0, raw=True
    ) -> Tuple[np.polynomial.Polynomial, np.polynomial.Polynomial]:
    ...
    def write_waveform_data(self, channel=1, filename=""):
        ...
        print(
            "Started saving waveform data for channel "
            + str(channel)
            + " "
            + str(mdepth)
            + " samples to filename "
